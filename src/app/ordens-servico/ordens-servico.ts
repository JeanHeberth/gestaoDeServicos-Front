    ...
    <div class="page">
      ...
      <div class="search-wrapper" role="search">
        <input
          ...
          placeholder="Buscar por código, descrição, cliente ou status"
      ...
      @if (loading) {
        ...
        <div class="card">
          @if (ordensFiltradas.length === 0) {
            <div class="no-results">Nenhuma ordem encontrada para "{{ busca }}".</div>
          } @else {
            <table class="table">
              <thead>
                <tr>
...
export class OrdensServicoComponent implements OnInit {
  ...
  get ordensFiltradas(): OrdemServico[] {
    ...
    return this.ordens.filter((ordem) => {
      const campos = [
        ...
        ordem.status,
        ordem.clienteId,
        this.clientesMap[ordem.clienteId],
      ];
  ...
  carregar() {
    ...
    forkJoin({
      ...
      next: ({ ordens, clientes }) => {
        // normalize date fields to Date objects so Angular DatePipe formats them correctly
        const parseDateField = (v: unknown) => {
        ...
        this.ordens = ordens.map((o: any) => ({
          ...o,
          dataAbertura: parseDateField(o.dataAbertura) || o.dataAbertura,
          dataFechamento: parseDateField(o.dataFechamento) || o.dataFechamento,
        }));
  ...
  novaOS() {
    // Navega para a tela de criação de Ordem de Serviço
    this.router.navigate(['/criar-ordem-servico']);
  }
  ...
  async editar(id: string) {
    ...
    // Formatter robusto para exibir data e hora legíveis no modal
    const formatSafe = (v: unknown) => {
      if (!v && v !== 0) return '—';
      try {
        const dp = new DatePipe('pt-BR');
        // forçar parse para Date primeiro (trata ISO e timestamps)
        const d = (v instanceof Date) ? v : new Date(String(v));
        if (isNaN(d.getTime())) return String(v);
        const asString = dp.transform(d, 'dd/MM/yyyy HH:mm');
        if (asString) return asString;
        // fallback para toLocaleString se DatePipe por algum motivo não funcionar
        return d.toLocaleString('pt-BR', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }).replace(/\u200E/g, '');
      } catch (e) {
        try {
    ...
    const formattedAbertura = formatSafe(os.dataAbertura);
    const formattedFechamento = formatSafe(os.dataFechamento);
    ...
    const result = await this.editModal.open({
      ...
      fields: [
        ...
        { key: 'dataAbertura', label: 'Data Abertura', type: 'text', value: formattedAbertura, disabled: true },
        { key: 'dataFechamento', label: 'Data Fechamento', type: 'text', value: formattedFechamento, disabled: true },
      ],
    });
